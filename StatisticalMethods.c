...
StatisticalReport analyze_numeric_data(double *data, int n, const char *outputDirectory, const char *fieldName)
{
	...
	// Write histogram to a file
	{
		...
		if (histFile) {
			...
			for (int i=0; i<hist.num_bins; i++)
			{
				...
				double bin_end = bin_start + hist.bin_width;
				fprintf(histFile, "%.17g %.17g %d\n", bin_start, bin_end, hist.bins[i]);
			}
			fclose(histFile);
		}
		free(hist.bins);
	...
	// Write summary statistics:
	{
		...
		if (statsFile) {
			fprintf(statsFile, "Mean: %.17g\n", mean);
			fprintf(statsFile, "Standard Deviation: %.17g\n", std_dev);
			fprintf(statsFile, "Skewness: %.17g\n", skew);
			fclose(statsFile);
		}
	}
	...
	double A2 = anderson_darling_normality_test(data, n);
	{
		...
		if (normFile) {
			fprintf(normFile, "Anderson-Darling A^2: %.17g\n", A2);
			fprintf(normFile, "Interpretation: Smaller A^2 = closer to normal.\n");
			fclose(normFile);
		}
	}
